 ...
 *
 * Почему zod, когда остальной вход валидируется class-validator. Проверка здесь
 ...
 *
 * Вторая причина: эти схемы — кандидат на переезд в packages/shared-types, чтобы клиент
 * проверял геометрию ТЕМ ЖЕ кодом, а не своей копией правил. class-validator туда не
 * переедет — он тащит за собой декораторы и reflect-metadata.
 */
 ...
 *
 * `satisfies Record<ElementType, ...>` — не украшение: добавят в enum схемы новую фигуру
 * (сейчас `rect | ellipse | line`), и этот файл перестанет компилироваться, пока для неё не
 * опишут геометрию. Без такой привязки новая фигура молча получила бы «валидацию» через
 * отсутствующую схему — то есть никакую.
 */
const ELEMENT_DATA_SCHEMAS = {
  rect: boxDataSchema,
 ...
 * превращается в 404 не там, где обнаружено, а там, где известен сценарий.
 */
