 ...
 *
 * Throttling здесь пока не подключён — сознательно, а не по забывчивости. У auth-роутов лимиты
 * висят на контроллере, потому что нужны точечно; правильный шаг для board-роутов — поднять
 * ThrottlerGuard до глобального с явным `@SkipThrottle()` на `/health` (иначе пробы
 * оркестратора начнут упираться в лимит и мигать readiness). Это отдельное решение и отдельная
 * таска, а не строчка попутно.
 */
@Controller('boards')
@Authorization()
