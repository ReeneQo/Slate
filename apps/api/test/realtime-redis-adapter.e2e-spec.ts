...
describe('Redis adapter (e2e, multi-instance)', () => {
  ...
  beforeEach(async () => {
    // Единственный сценарий этого файла не полагался на пустую базу, пока контейнер был свой.
    // Теперь Postgres/Redis общие на весь e2e-прогон (SLT-48) — без сброса тест зависел бы от
    // того, что осталось от файла, отработавшего перед этим (порядок файлов не гарантирован).
    await apps.reset();
  });
